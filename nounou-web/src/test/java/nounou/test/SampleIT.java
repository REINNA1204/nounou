...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "nounou-ear.ear");

        // Current directory is the root of the "nounou-web" project. Go up one level
        // and enter the "nounou-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../nounou-ejb/target/nounou-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "nounou-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/nounou-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "nounou-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
